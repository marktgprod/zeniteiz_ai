...
from app.services.loyalty import TIER_RANK, check_level_up, get_effective_tier
...
async def generate_flux(payload: ImageGenerateRequest, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    if TIER_RANK[effective_tier] < TIER_RANK[SubscriptionTier.PRO]:
        raise HTTPException(status_code=403, detail="Изображения доступны начиная с тарифа Pro")

    limit = REQUEST_LIMITS.get(effective_tier, 0)
...
async def generate_dalle3(payload: ImageGenerateRequest) -> dict:
    raise HTTPException(status_code=501, detail="DALL-E 3 ещё не подключён — используйте Flux.1 Pro")


@router.post("/api/image/generate-prompt")
async def generate_prompt(payload: ImageGenerateRequest) -> dict:
    raise HTTPException(status_code=501, detail="Промпт-генератор ещё не подключён")
