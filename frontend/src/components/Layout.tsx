...
export default function Layout() {
  ...
  return (
    <div className="min-h-svh bg-white text-gray-900 dark:bg-black dark:text-gray-100">
      {/* Desktop sidebar — hidden on phone, where the top tab strip below is used instead */}
      ...
      <div className="flex min-h-svh flex-col lg:pl-56">
        {/* Mobile-only top bar + tab strip, stacked in one sticky block — desktop
            uses the sidebar instead. Nav lives up here (not as a fixed bottom bar)
            so a vertical scroll gesture on the page content never starts on top of
            it and gets misread as a horizontal nav swipe / accidental tab switch. */}
        <div className="sticky top-0 z-10 lg:hidden">
          <header className="flex items-center justify-between border-b border-gray-200/80 bg-white/90 px-4 py-3 backdrop-blur-md dark:border-white/10 dark:bg-black/90">
          ...
          <nav className="flex gap-1.5 overflow-x-auto border-b border-gray-200/80 bg-white/90 px-3 py-2 backdrop-blur-md [-ms-overflow-style:none] [scrollbar-width:none] dark:border-white/10 dark:bg-black/90 [&::-webkit-scrollbar]:hidden">
            {NAV_ITEMS.map((item) => {
              const Icon = item.icon
              return (
                <NavLink
        ...
        <main className="flex-1 overflow-y-auto pb-8 lg:pb-10">
          <Outlet />
        </main>
      </div>
    </div>
